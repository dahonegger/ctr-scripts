...
%
% Type: Function [Cube]=ReadRawRadarBin(fname,radar_loc,intoption,xdomR,ydomR)
%
...
function [Cube]=ReadRawRadarBinCOR(fname,radar_loc,intopt,xdomR,ydomR,tdom)

warning('off');
%%
%check the doughnut
if radar_loc(4)==0
    disp('You passed a doughnut size of 0. I need at least a value of 1. I changed it')
    radar_loc(4)=1;
end
%check the number of arguments and decide the interpolation options
switch nargin
    case 2
        intopt=0;
    case 4
        ydomR=xdomR;
        xdomR=intopt; %rearrange the variables
        intopt=2;
end
%add a zero elevation if not provided, to calculate the ground range
if numel(radar_loc)~=5
    radar_loc(5)=0;
end
%% opening the file
