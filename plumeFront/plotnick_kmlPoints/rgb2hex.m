...
%
% Modified by Dan Plotnick to return the 8-digit Hex code needed for Google
% Earth. This also reverses the rgb order, so the byte order is
% [alpha,b,r,g]. Some of the above description may thus be wrong. 
...
assert(max(alpha(:)) <=1 & min(alpha(:))>=0,'Alpha values must be on range [0, 1]');

if length(alpha) ~= 1
    if length(alpha) ~= size(rgb,1)
        error('Size of alpha must be 1 or the length of the rgb vector');
    end
    % Flip Alpha
    if size(alpha,1) ~= size(rgb,1)
        alpha = alpha.';
    end
else
    alpha = repmat(alpha,size(rgb,1),1);
end
...
%% Flip RGB to BRG. Thanks Google Earth. :-(
rgb = fliplr(rgb);


%% Convert (Thanks to Stephen Cobeldick for this clever, efficient solution):
hex =  reshape(sprintf('%02X',[alpha,rgb].'),8,[]).'; 
% hex(:,3:8) = reshape(sprintf('%02X',[rgb.'),6,[]).'; 
% hex(:,1:2) = reshape(sprintf('%X',alpha.'),2,[]).'; 
