...
% Copyright (c)1995-2014, Dr. Erik A. Johnson <JohnsonE@usc.edu>, 7/16/2014
...
firstprop = nargin+1;
for k=1:length(varargin), if ~isnumeric(varargin{k}), firstprop=k; break; end
end
    ...
    elseif strncmp(prop,'userd'  ,5),   %ignore it
        ...
        try
            ...
            try
                ...
                errstr(1:find(errstr==char(13)|errstr==char(10), 1, 'last' )) = '';
...
if ~isempty(oldh),
    ...
    if any(mask),
        ...
        oldh = cat(1,oldh{:});
        if isempty(oldh), return
        end % no arrows to modify, so just leave
    end
end
...
while (any(axnotdone)),
    ...
    tmp=ax==curax;
    oldaxlims(:,tmp)=repmat(reshape(axl',[],1),1,sum(tmp));
    ...
    try curT=get(curax,'Xform');
    catch
        tmp=num2cell(get(curax,'View'));
        curT=viewmtx(tmp{:});
    end
...
tmp=(pts(:,2*narrows+1:3*narrows)+pts(:,3*narrows+1:4*narrows))/2;              %eaj 7/14/2014
pts = [tmp pts(:,[3*narrows+1:end narrows+1:3*narrows]) tmp];               %eaj 7/14/2014
...
if (nargout<=1),
    ...
    for k=1:narrows,
        ...
        if newpatch(k) || ispatch(k) %eaj 7/14/2014
...
function [H,oldaxlims,errstr] = arrow_clicks(H,ud,x,~,~,ax,oldaxlims)
% handle choosing arrow Start and/or Stop locations if necessary
errstr = '';
if isempty(H)||isempty(ud)||isempty(x), return
end
...
if (dolog), start(:,2)=10.^start(:,2); stop(:,2)=10.^stop(:,2); end
h2 = feval(mfilename,start,stop,24,[90;60;120],[],[0;0;4],'Ends',char('both','stop','stop'));
