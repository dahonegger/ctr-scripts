...
%
% Type: Function [Cube]=ReadBinTri(fname,site,intoption,xdomR,ydomR,nframes)
%
% Call ins: readAfileTime.m
%
% 	EXAMPLE:
%
% [Cube]=readBinTri(1890000,'params_NRI',2,-1000:10:1000,-1000:10:1000,50);
%
%
% 	INPUT:
%
...
function [Cube]=readBinTri(fname,site,intopt,xdomR,ydomR,nframes)
...
%%
%for debugging, we use fewer header rotations
...
%         ===========
%         TriScatteredInterp Version
        interpFunc = TriScatteredInterp(XX(:),YY(:),ZZ(:));
        DR = interpFunc(X_D,Y_D);
%         ===========
...
%         ==========
    ...
    elseif intopt==1 || intopt==3 || intopt==4
        ...
        %%%%% NOT AVAILABLE WHILE PARALLELIZED %%%%%
...
%     ShowProgress(jj,header.rotations,5);
    for dels = 1:length(str)
        fprintf('\b');
    end
    str = sprintf('%s: %d/%d Done at %s',fname,jj,header.rotations,datestr(now));
    fprintf('%s',str)
